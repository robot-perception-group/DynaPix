...
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const cv::Mat &imProb, const cv::Mat &imMask, const cv::Mat &imRGB,
             const double &timeStamp,  ORBextractor* extractor, ORBVocabulary* voc,
             cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth)
    :mImMask(imMask), mpORBvocabulary(voc), mpORBextractorLeft(extractor), mpORBextractorRight(static_cast<ORBextractor*>(NULL)), mImGray(imGray),
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth), mIsKeyFrame(false), mImDepth(imDepth), mImRGB(imRGB), mImProb(imProb)
{
    // Frame ID
    mnId=nNextId++;
