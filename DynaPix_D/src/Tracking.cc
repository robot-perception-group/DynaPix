...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const cv::Mat &imP, cv::Mat &mask,
                                ...
                                cv::Mat &imDOut, cv::Mat &maskOut)
{
    ...
    imProb.convertTo(imProb, CV_32F, 1.0f/1000.0);
    mCurrentFrame = Frame(mImGray,imDepth,imProb, imMask,_imRGB,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
    ...
    mCurrentFrame = Frame(mImGray,imDepth,imProb,imMask,imRGBOut,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
...
void Tracking::CreateNewKeyFrame()
{
    ...
    if(mSensor!=System::MONOCULAR)
    {
        ...
        pKF->mImProb = mCurrentFrame.mImProb.clone();
        if(!vDepthIdx.empty())
        {
            sort(vDepthIdx.begin(),vDepthIdx.end());
            ...
            for(size_t j=0; j<vDepthIdx.size();j++)
            {
                ...
                if(bCreateNew)
                {
                    ...
                    int val = (int)mCurrentFrame.mImMask.at<uchar>(v, u);
                    if (val==1){
    ...
    while(nCandidates>0 && !bMatch)
    {
        for(int i=0; i<nKFs; i++)
        {
            ...
            if(!Tcw.empty())
            {
                ...
                int nGood = Optimizer::PoseWeightOptimization(&mCurrentFrame);
                ...
                if(nGood<50)
                {
                    ...
                    {
                        nGood = Optimizer::PoseWeightOptimization(&mCurrentFrame);
