# DynaPix-D SLAM
This section contains the extended deployment of our introduced methodologies on **DynaSLAM** framework, involving inclusion of **estiamted motion probability** and **improved pose optimization process**.

## Prerequisties
Similar to the original DynaSLAM, the prerequisties for DynaPix-D involves Python 2.7, Eigen 3, C++11 compiler, gcc/g++ 4.8, Pangolin-0.5, and OpenCV 2.4.11.

## Install
- Compile DynaPix-D source code
  ```
...
- Install required libaries for Mask-RCNN
  ...
  # Tested with CUDA 9.0/CUDNN 7.0
  ```
- Download pretrained model mask_rcnn_coco.h5 [here](https://github.com/matterport/Mask_RCNN/releases/download/v2.0/mask_rcnn_coco.h5) and place it in the folder `src/python`

- Check Mask-RCNN work properly by running python2 `src/python/Check.py`.   If everything is alright, it will print Mask R-CNN is correctly working.
## Run
- Execute the following command to start testing. Change specific variables for target sequence, then the estimated trajecotry will be saved as `CameraTrajectory.txt`.
  ```
  ./Examples/RGB-D/rgbd_tum Vocabulary/ORBvoc.txt ${SEQ_YAML_FILE} ${PATH_TO_SEQUENCE} ${ASSOCIATIONS_FILE} ${PATH_TO_MASKS} ${PATH_TO_RESULTS}
  ```
- TUM-RGBD Example:
  ```
  ./Examples/RGB-D/rgbd_tum Vocabulary/ORBvoc.txt Examples/RGB-D/TUM3.yaml exp1/ exp1/halfsphere.txt masks/ results/
  ```
- GRADE Example:
  ```
  ./Examples/RGB-D/rgbd_tum Vocabulary/ORBvoc.txt Examples/RGB-D/GRADE.yaml exp2/ exp2/rgbd_assoc.txt masks/ results/
  ```

For more details, you can follow the instructions below:
___
## DynaSLAM
